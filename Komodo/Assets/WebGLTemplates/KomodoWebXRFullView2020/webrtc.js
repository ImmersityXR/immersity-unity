// WebRTC mesh voice/video/screen-share for Komodo sessions.
//
// Ported from David Tamayo's KomodoSandbox prototype:
//   https://github.com/davtamay/KomodoSandbox (client)
//   https://github.com/davtamay/RelayTesting  (original signaling server)
// Adapted for this project:
// - connects to the relay's dedicated /rtc namespace (komodo-relay rtc.js)
//   with Socket.IO 2.x query-parameter handshake (userName, client_id,
//   session_id, auth)
// - ICE servers (STUN/TURN) come from the server via the 'rtcConfig' event
//   instead of being hardcoded here
// - requires relay.js to be loaded first (uses RELAY_BASE_URL and params)
//
// Unity entry points are exposed through webrtcUnity.jslib; incoming events
// are delivered with SendMessage to window.socketIOAdapterName (see
// docs/WEBRTC-PORT.md).
...
async function ConnectToWebRTCSocket(name) {
    ...
    if (!socket) {
        // Socket.IO 2.x handshake: connection metadata travels as query
        // parameters (handshake.auth is a Socket.IO 4 feature). The relay's
        // /rtc namespace scopes signaling to this Komodo session and, when
        // configured, requires the shared client secret (page ?auth= param).
        socket = io(RELAY_BASE_URL + '/rtc', {
            query: {
                userName: name,
                client_id: window.client_id || '',
                session_id: (typeof params !== 'undefined' && params.session) ? params.session : '',
                auth: (typeof params !== 'undefined' && params.auth) ? params.auth : ''
            }
        });
...
function setupSocketListeners() {
    ...
    // STUN/TURN servers are configured centrally on the relay
    // (config.rtc.iceServers) and delivered on connection, so per-deployment
    // TURN credentials never need to be baked into builds.
    socket.on('rtcConfig', config => {
        if (config && config.iceServers && config.iceServers.length) {
            peerConfiguration.iceServers = config.iceServers;
        }
    });
